...
class MatchingEngine:
    ...
    def handle_order(self, order):
        ...
        Returns:
            list: The filled orders produced by the relevant handler.
        """
            ...
            while self.ask_book:
                ...
                # Correct stock and order price less/equal to price in book
                    ...
                    # Book Quantity > Order Quantity
                        ...
                        # Fill the order
                        # Record the fill as a snapshot: `order` is mutated below
                        # (and possibly posted to the book), so appending the live
                        # object would report a quantity of 0.
                        filled_orders.append(copy.deepcopy(order))
                        filled_orders.append(book)
                        order.quantity = 0

                    # Order Quantity == Book Quantity
                    else:
                        # Snapshot both sides before zeroing their quantities.
                        filled_orders.append(copy.deepcopy(book))
                        filled_orders.append(copy.deepcopy(order))
                        order.quantity = 0
                        book.quantity = 0
    ...
    def insert_limit_order(self, order):
        ...
        # Price-time priority: best price first, earliest order first at a price.
        self.bid_book = sorted(self.bid_book, key=lambda x: (-x.price, x.time))
        self.ask_book = sorted(self.ask_book, key=lambda x: (x.price, -x.time))
    ...
    def amend_quantity(self, id, quantity):
        ...
        if not found_order:
            for index, ask in enumerate(self.ask_book):
                if ask.id == id:
                    found_order = True
                    if quantity > ask.quantity:
                        # Raise if the user tries to modify an order to a
                        # with a quantity that's greater than given in the existing order
                        raise NewQuantityNotSmaller("Amendment Must Reduce Quantity!")
                    else:
                        self.ask_book[index].quantity = quantity
